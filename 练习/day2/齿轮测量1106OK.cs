...
public partial class HDevelopExport
{
  ...
  public void FindMatch (HObject ho_Image1, HObject ho_ModelRegion, out HObject ho_TransContours, 
      HTuple hv_ModelID, HTuple hv_Start, HTuple hv_Extent, HTuple hv_MinScore, HTuple hv_MatchNum, 
      HTuple hv_MaxOverlap, out HTuple hv_AlignmentHomMat2D, out HTuple hv_Error)
  {
    ...
    try
    {
      ...
      HOperatorSet.FindShapeModel(ho_Image1, hv_ModelID, hv_Start.TupleRad(), hv_Extent.TupleRad()
          , hv_MinScore, hv_MatchNum, hv_MaxOverlap, "least_squares", (new HTuple(7)).TupleConcat(
      ...
      //创建ROI跟随的仿射变换矩阵，起始点坐标是模板区域的中心（RefRow，RefColumn）
      //相当于我们之前用的vector_angle_to_rigid建立的仿射变换矩阵
      //由匹配助手导出的ROI跟随的仿射变换矩阵，只能对ROI、XLD，点坐标进行仿射变换，不能对图像进行仿射变换
      ...
      //vector_angle_to_rigid (RefRow, RefColumn, 0, Row[I], Column[I], Angle[I], HomMat2D1)
  ...
  private void action()
  {
    ...
    HObject ho_Image, ho_Regions, ho_Regions1;
    HObject ho_Circle, ho_ModelRegion, ho_ModelContours, ho_TransContours;
    HObject ho_Image1=null, ho_Regions2=null, ho_Circle1=null;

    // Local control variables 

    HTuple hv_Scale = new HTuple(), hv_zhijing = new HTuple();
    HTuple hv_WindowHandle = new HTuple(), hv_ROIRows = new HTuple();
    HTuple hv_ROICols = new HTuple(), hv_Direct = new HTuple();
    HTuple hv_ResultRow = new HTuple(), hv_ResultColumn = new HTuple();
    HTuple hv_ArcType = new HTuple(), hv_RowCenter = new HTuple();
    HTuple hv_ColCenter = new HTuple(), hv_Radius = new HTuple();
    HTuple hv_StartPhi = new HTuple(), hv_EndPhi = new HTuple();
    HTuple hv_PointOrder = new HTuple(), hv_ArcAngle = new HTuple();
    ...
    HTuple hv_Index = new HTuple(), hv_AlignmentHomMat2D = new HTuple();
    HTuple hv_Error = new HTuple(), hv_RowTrans = new HTuple();
    HTuple hv_ColTrans = new HTuple(), hv_ResultRow1 = new HTuple();
    HTuple hv_ResultColumn1 = new HTuple(), hv_ArcType1 = new HTuple();
    HTuple hv_RowCenter1 = new HTuple(), hv_ColCenter1 = new HTuple();
    HTuple hv_Radius1 = new HTuple(), hv_StartPhi1 = new HTuple();
    HTuple hv_EndPhi1 = new HTuple(), hv_PointOrder1 = new HTuple();
    HTuple hv_ArcAngle1 = new HTuple();
    // Initialize local and output iconic variables 
    ...
    try
    {
      ...
      ho_Image.Dispose();
      HOperatorSet.ReadImage(out ho_Image, "D:/A机器视觉/Halcon/2hao20150107190908.bmp");
      //
      if (HDevWindowStack.IsOpen())
      {
      ...
      //*** 设置ROI，拟合圆***********
      ho_Regions.Dispose();hv_ROIRows.Dispose();hv_ROICols.Dispose();hv_Direct.Dispose();
      draw_spoke3(ho_Image, out ho_Regions, hv_WindowHandle, 30, 60, 15, out hv_ROIRows, 
          out hv_ROICols, out hv_Direct);
      ho_Regions1.Dispose();hv_ResultRow.Dispose();hv_ResultColumn.Dispose();hv_ArcType.Dispose();
      spoke(ho_Image, out ho_Regions1, 30, 60, 15, 1, 20, "all", "max", hv_ROIRows, 
          hv_ROICols, "inner", out hv_ResultRow, out hv_ResultColumn, out hv_ArcType);
      ho_Circle.Dispose();hv_RowCenter.Dispose();hv_ColCenter.Dispose();hv_Radius.Dispose();hv_StartPhi.Dispose();hv_EndPhi.Dispose();hv_PointOrder.Dispose();hv_ArcAngle.Dispose();
      ...
      ho_ModelRegion.Dispose();
      DrawMatch(out ho_ModelRegion, hv_WindowHandle);
      //
      //Matching 01: Reduce the model template
      ho_ModelContours.Dispose();hv_ModelID.Dispose();
      CreateMatch(ho_Image, ho_ModelRegion, out ho_ModelContours, 0, 360, out hv_ModelID);
      //
      ...
      HOperatorSet.ListFiles("D:/A机器视觉/Halcon/案例/7 案例分析图片/4.案例分析图片/现场班-测量/齿轮测量", 
          ...
          ))-1); hv_Index = (int)hv_Index + 1)
      {
        ...
        ho_TransContours.Dispose();hv_AlignmentHomMat2D.Dispose();hv_Error.Dispose();
        FindMatch(ho_Image1, ho_ModelRegion, out ho_TransContours, hv_ModelID, 0, 
            360, 0.5, 1, 0.5, out hv_AlignmentHomMat2D, out hv_Error);
        hv_RowTrans.Dispose();hv_ColTrans.Dispose();
        HOperatorSet.AffineTransPixel(hv_AlignmentHomMat2D, hv_ROIRows, hv_ROICols, 
            out hv_RowTrans, out hv_ColTrans);
        ho_Regions2.Dispose();hv_ResultRow1.Dispose();hv_ResultColumn1.Dispose();hv_ArcType1.Dispose();
        spoke(ho_Image1, out ho_Regions2, 30, 60, 15, 1, 20, "all", "max", hv_RowTrans, 
            hv_ColTrans, "inner", out hv_ResultRow1, out hv_ResultColumn1, out hv_ArcType1);
        ho_Circle1.Dispose();hv_RowCenter1.Dispose();hv_ColCenter1.Dispose();hv_Radius1.Dispose();hv_StartPhi1.Dispose();hv_EndPhi1.Dispose();hv_PointOrder1.Dispose();hv_ArcAngle1.Dispose();
        pts_to_best_circle(out ho_Circle1, hv_ResultRow1, hv_ResultColumn1, 3, "circle", 
        ...
        using (HDevDisposeHelper dh = new HDevDisposeHelper())
        {
        hv_zhijing = (hv_Scale*hv_Radius1)*2;
        }
        if (HDevWindowStack.IsOpen())
        {
          HOperatorSet.SetColor(HDevWindowStack.GetActive(), "green");
        ...
        using (HDevDisposeHelper dh = new HDevDisposeHelper())
        {
        disp_message(hv_WindowHandle, "直径="+hv_zhijing, "window", 20, 20, "red", 
            "true");
        }
        // stop(...); only in hdevelop
      }
